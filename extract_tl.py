...
if __name__ == '__main__':
    ...
    model = "/home/lynx/Documents/traffic_light_classifier/models/amy9.onnx"
    class_names = ['oneway', 'highwayexit', 'stopsign', 'roundabout', 'park', 'crosswalk', 'noentry', 'highwayentrance', 'priority', 'light', 'block', 'girl', 'car']
    detector = InferenceModel(model, conf_thres=0.45, iou_thres=0.35)

    counter = 0
    saveFolder = "/home/lynx/Documents/traffic_light_classifier/tl_detected/"
    sourceImgs = os.listdir("/home/lynx/Pictures/lights")
    for imgpath in sourceImgs:
        print(imgpath)
        image = cv2.imread("/home/lynx/Pictures/lights/" + imgpath)
        boxes, scores, class_ids = detector(image)
        img = draw_detections(image, boxes, scores, class_ids)
        # cv2.rectangle(image, (100, 100), (200, 300), (255,0,0), 2)
        # cv2.imshow('sign', img)
        # cv2.waitKey(0)
        cv2.imwrite(saveFolder + str(counter) +(".jpg"),img)
        counter += 1
